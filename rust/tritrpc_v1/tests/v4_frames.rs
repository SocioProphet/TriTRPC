...
fn v4_frames_hot_unary_positive_cases_match_fixture() {
    ...
    let positive: Vec<&FixtureCase> = file.cases.iter().filter(|c| c.kind == "positive").collect();
    assert!(
        !positive.is_empty(),
        "no positive cases found in hot_unary.json"
    );
    ...
    for case in positive {
        let input = case
            .semantic_input
            .as_ref()
            .unwrap_or_else(|| panic!("case {} missing semantic_input", case.id));
        ...
        let tag_bytes =
            hex::decode(tag_hex).unwrap_or_else(|e| panic!("case {}: bad tag_hex: {e}", case.id));
        let mut tag = [0u8; 16];
        ...
        let expected_hex = case
            .expected_frame_hex
            .as_deref()
            .unwrap_or_else(|| panic!("case {} missing expected_frame_hex", case.id));
...
fn v4_frames_hot_unary_negative_cases_produce_errors() {
    ...
    let negative: Vec<&FixtureCase> = file.cases.iter().filter(|c| c.kind == "negative").collect();
    assert!(
        !negative.is_empty(),
        "no negative cases found in hot_unary.json"
    );
    ...
    for case in negative {
        ...
        assert_eq!(
            op, "parse",
            "case {}: only 'parse' operation supported",
            case.id
        );
        ...
        let input_hex = case
            .input_hex
            .as_deref()
            .unwrap_or_else(|| panic!("case {} missing input_hex", case.id));
...
fn v4_frames_stream_open_positive_cases_match_fixture() {
    ...
    assert!(
        !positive.is_empty(),
        "no positive cases found in stream_open.json"
    );
...
fn v4_frames_stream_open_negative_cases_produce_errors() {
    ...
    assert!(
        !negative.is_empty(),
        "no negative cases found in stream_open.json"
    );
