...
This draft specifies how policy/evidence references are serialized into the existing TritRPC
AUX field without changing the stable TritRPC v1 wire layout.

## Requirements

1. Implementations **MUST** treat the profile payload as a JSON object.
2. Implementations **MUST** canonicalize that object using RFC 8785 (JCS) before hashing,
   signing, or receipt verification.
...
5. When AEAD is enabled, the existing AAD definition applies; therefore AUX bytes are
   authenticated as part of the frame.
6. Hash-typed identifiers inside the profile **SHOULD** use stable `sha256:` identifiers for
   long-lived semantic/governance references.
7. This profile **MUST NOT** be interpreted as the semantic policy engine; it is a carriage
   profile only.
