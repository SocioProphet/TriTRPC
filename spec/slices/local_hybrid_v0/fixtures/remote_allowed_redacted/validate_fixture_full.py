#!/usr/bin/env python3
"""Validate the full seven-step remote_allowed_redacted fixture examples.

This validator intentionally uses only the Python standard library. It reads
`verification-manifest.full.json`, loads the referenced JSON examples in the
same directory, and evaluates a very small assertion set.
"""
...
MANIFEST_PATH = FIXTURE_DIR / "verification-manifest.full.json"
...
def validate_target(target: dict[str, Any]) -> tuple[bool, str]:
    ...
    if "equals" in expect and actual != expect["equals"]:
