...
This v2 variant fixes the repository root calculation so the script is runnable
from the actual repo layout. It also fixes the suite to the reference fallback
MAC behavior so the packed artifact stays stable across environments.
"""
...
def build_packed_vector() -> dict[str, Any]:
    ...
    tag = hashlib.blake2b(aad, key=key, digest_size=16).digest()
