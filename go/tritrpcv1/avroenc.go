...
func EncBool(v bool) []byte  { if v { return []byte{1} } ; return []byte{0} }
...
func EncArray(items [][]byte) []byte {
    if len(items) == 0 { return []byte{0} }
    out := EncLong(int64(len(items)))
    for _, it := range items { out = append(out, it...) }
    out = append(out, 0)
    return out
}
func EncMap(m map[string]string) []byte {
    if len(m) == 0 { return []byte{0} }
...
func EncHello(modes, suites, comp []string, contextURI *string) []byte {
    arr := func(ss []string) []byte {
        ...
        for _, s := range ss { chunks = append(chunks, EncString(s)) }
...
func EncHyperedge(eid string, members []string, weight *int64, attr map[string]string) []byte {
    ...
    for _, m := range members { items = append(items, EncString(m)) }
